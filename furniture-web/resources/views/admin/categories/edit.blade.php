...
@section('content')
    <section class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold tracking-tight">Edit Kategori</h1>
            <p class="mt-1 text-sm text-slate-600">Perbarui data kategori.</p>
        </div>

        <div class="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm">
            <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="grid gap-4">
                @csrf
                @method('PUT')

                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700">Nama kategori</label>
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm outline-none ring-brand-200 focus:ring-4">
                ...
                <div>
                    ...
                    @if ($category->image_url)
                        <div class="mb-3">
                            <img src="{{ $category->image_url }}" alt="{{ $category->name }}" class="h-32 w-32 rounded-xl object-cover">
                            <p class="mt-1 text-xs text-slate-500">Gambar saat ini. Upload baru untuk mengganti.</p>
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm outline-none ring-brand-200 focus:ring-4">{{ old('description', $category->description) }}</textarea>
